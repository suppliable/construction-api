...
const getCodThreshold = async (req, res) => {
  try {
    ...
    res.json({ success: true, cod_threshold: settings.cod_threshold ?? 7500 });
  } catch (err) {
...
const updateCodThreshold = async (req, res) => {
  try {
    ...
    res.json({ success: true, cod_threshold: parseFloat(value) });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
};
...
const updateWarehouseStatus = async (req, res) => {
  try {
    ...
    if (isOpen === undefined || isOpen === null) {
      return res.status(400).json({ success: false, message: 'isOpen is required' });
    }
