...
const { updateDeliveryConfig, getDeliveryConfig } = require('../services/firestoreService');

const calculateDeliveryCharge = async (req, res) => {
  try {
    const { pincode, latitude, longitude, orderValue } = req.body;
    ...
    const result = await calculateDelivery(pincode, parseFloat(latitude), parseFloat(longitude), parseFloat(orderValue) || 0);
